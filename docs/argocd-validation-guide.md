...
## Handling Stuck Sync Operations

### Problem Overview

Argo CD sync operations can become stuck due to:
- Deadlocked hooks or pre-sync jobs
- Resource conflicts in the cluster
- Network issues during sync
- Corrupted operation state

Stuck operations block new deployments and cause workflow timeouts (typically after 5+ minutes).

### Automatic Cleanup

The deployment system includes automatic cleanup of stuck operations:

**In CI/CD Workflows:**
- Pre-deployment cleanup step runs before validation
- Automatically aborts operations running >5 minutes
- Ensures clean state before new deployments

**In Validators:**
- Auto-cleanup enabled in GitHub Actions (`GITHUB_ACTIONS` env var)
- Can be explicitly enabled with `AUTO_CLEANUP_STUCK_OPS=true`
- Logs cleanup actions for audit trail

**In Reusable Action:**
```yaml
- name: Cleanup stuck operations
  uses: ./.github/actions/cleanup-argocd-operation
  with:
    app-name: yt-summarizer-prod
    namespace: argocd
    timeout-threshold: '300'  # 5 minutes
```

### Manual Cleanup

If automatic cleanup fails or you need to manually intervene:
...
**2. Abort stuck operation:**
```bash
kubectl patch application yt-summarizer-prod -n argocd \
...
# Should output nothing or "None"
```

**4. Check for stuck resources:**
```bash
# Check for stuck hooks/jobs
kubectl get jobs -n yt-summarizer
...
1. **Set appropriate timeouts:**
   - Hook timeouts: `argocd.argoproj.io/hook-delete-policy: HookSucceeded`
   - Sync timeouts in application spec
   - Health check timeouts for custom resources

2. **Use idempotent hooks:**
   - PreSync hooks should be safe to run multiple times
   - Include cleanup logic in hook failures

3. **Monitor operation duration:**
   ```bash
   kubectl get applications -n argocd \
     -o custom-columns=NAME:.metadata.name,OPERATION:.status.operationState.phase,STARTED:.status.operationState.startedAt
   ```

4. **Configure auto-sync wisely:**
   - Disable auto-sync during initial setup
   - Test sync manually first
   - Enable after validation

### Troubleshooting Stuck Operations

**Operation stuck during PreSync hook:**
```bash
# Find stuck hook jobs
kubectl get jobs -n yt-summarizer -l argocd.argoproj.io/instance=yt-summarizer-prod

# Delete stuck hook job
kubectl delete job <hook-job-name> -n yt-summarizer

# Retry sync
argocd app sync yt-summarizer-prod
```

**Operation stuck during resource application:**
```bash
# Check for pending resources
kubectl get all -n yt-summarizer

# Look for resources in pending state
kubectl describe <resource-type> <resource-name> -n yt-summarizer

# Check Argo CD controller logs
kubectl logs -n argocd deployment/argocd-application-controller --tail=100
```

**Operation stuck indefinitely (>30 minutes):**
```bash
# Force abort and restart Argo CD controller
kubectl patch application yt-summarizer-prod -n argocd --type merge -p '{"operation":null}'
...
**Inputs:**
- `app-name` (required): Argo CD application name
- `namespace` (default: argocd): Argo CD namespace
- `timeout-threshold` (default: 300): Seconds before operation is considered stuck
- `force` (default: false): Force cleanup even if not stuck

**Outputs:**
- `cleaned`: Whether a stuck operation was cleaned (true/false)
- `operation-state`: Operation state before cleanup
...
- name: Check and cleanup
  ...
  with:
    app-name: yt-summarizer-prod
    namespace: argocd
    timeout-threshold: '180'  # 3 minutes

- name: Handle cleanup result
  if: steps.cleanup.outputs.cleaned == 'true'
  run: echo "Cleaned up stuck operation"
```
