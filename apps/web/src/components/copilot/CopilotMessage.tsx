...
export function CopilotMessage({
  ...
  const deriveScopeLevel = (): { label: string; icon: 'video' | 'channel' | 'library' } | null => {
    ...
    if (scopeEcho.videoIds && scopeEcho.videoIds.length > 0) {
      return {
        label:
          scopeEcho.videoIds.length === 1 ? 'This Video' : `${scopeEcho.videoIds.length} Videos`,
        icon: 'video',
      };
    }
    ...
    if (scopeEcho.channels && scopeEcho.channels.length > 0) {
      return {
        label:
          scopeEcho.channels.length === 1
            ? 'This Channel'
            : `${scopeEcho.channels.length} Channels`,
        icon: 'channel',
      };
    }
  ...
  return (
    <div className="w-full space-y-3 animate-in fade-in duration-200">
            ...
            <>
              ...
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-[var(--copilot-kit-primary-color)]/10 border border-[var(--copilot-kit-primary-color)]/20">
                {scopeInfo.icon === 'video' && (
                  <svg
                    className="w-3 h-3 text-[var(--copilot-kit-primary-color)]"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <span className="font-medium text-[var(--copilot-kit-primary-color)]">
                  {scopeInfo.label}
                </span>
              </span>
            </>
          ...
          {evidence.length > 0 && (
            <div className="mt-3 pt-3 border-t border-[var(--copilot-kit-separator-color)]">
              <div className="flex items-center gap-2 mb-2">
                <svg
                  className="w-4 h-4 text-[var(--copilot-kit-primary-color)]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <ol className="space-y-2">
                {evidence.map((ev, index) => (
                  <li key={ev.segmentId} className="text-sm flex items-start gap-2 group/cite">
                    <span className="text-[var(--copilot-kit-primary-color)] font-semibold shrink-0 bg-[var(--copilot-kit-secondary-color)] rounded px-1.5 py-0.5 text-xs border border-[var(--copilot-kit-separator-color)]">
                      [{index + 1}]
                    </span>
                    <a
                      href={ev.youTubeUrl}
                    ...
                    >
                      ...
                      <span className="text-[10px] text-[var(--copilot-kit-muted-color)]">
                        @ {formatDuration(ev.startTime)}
                      </span>
                      <svg
                        className="w-3 h-3 text-[var(--copilot-kit-muted-color)] group-hover/cite:text-[var(--copilot-kit-primary-color)] shrink-0"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
      ...
      {followups && followups.length > 0 && (
        <FollowupButtons suggestions={followups} onFollowupClick={onFollowupClick || (() => {})} />
      )}
    </div>
  );
}
