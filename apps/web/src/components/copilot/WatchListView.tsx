    ...
    >
      ...
      {isExpanded && (
        <div className="mt-4 ml-0 sm:ml-14 space-y-4">
          {/* Description */}
          <p className={`${copilotTextSizes.body} ${copilotColors.primary}`}>{item.description}</p>
          ...
          >
            ...
            <p className={`${copilotTextSizes.small} ${copilotColors.primary}`}>{item.reason}</p>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <h3 className={`${copilotTextSizes.header} ${copilotColors.primary}`}>{watchList.title}</h3>
      ...
      <div className="space-y-4">
        {priorityGroups.map((group) => (
          <div key={group.priority} className="space-y-2">
            <h4
              className={`${copilotTextSizes.small} ${copilotColors.muted} font-medium uppercase tracking-wide`}
            >
              {group.priority} Priority
            </h4>
            <div className="space-y-2">
              {group.items.map((item) => (
                <WatchListItemCard key={item.videoId} item={item} onVideoClick={onVideoClick} />
              ))}
            </div>
          </div>
        ))}
