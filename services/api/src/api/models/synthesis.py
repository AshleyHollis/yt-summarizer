...
class LearningPathEvidence(BaseModel):
    ...
    segment_id: UUID | None = Field(
        default=None, alias="segmentId", description="Relevant segment ID"
    )
    segment_text: str | None = Field(
        default=None, alias="segmentText", description="Relevant transcript text"
    )
    youtube_url: str | None = Field(
        default=None, alias="youTubeUrl", description="YouTube URL with timestamp"
    )
...
class LearningPathItem(BaseModel):
    ...
    prerequisites: list[int] = Field(
        default_factory=list, description="Order numbers of prerequisite items"
    )
    evidence: list[LearningPathEvidence] = Field(
        default_factory=list, description="Evidence supporting this item's placement"
    )
...
class LearningPath(BaseModel):
    ...
    estimated_duration: int = Field(
        alias="estimatedDuration", description="Total duration in seconds"
    )
    items: list[LearningPathItem] = Field(
        default_factory=list, description="Ordered list of videos in the path"
    )
    coverage_summary: str = Field(
        alias="coverageSummary", description="Summary of what topics are covered"
    )
    gaps: list[str] = Field(
        default_factory=list, description="Topics not covered - 'what's missing' messaging"
    )
...
class WatchListItem(BaseModel):
    ...
    thumbnail_url: str | None = Field(default=None, alias="thumbnailUrl")
    duration: int | None = Field(default=None, description="Duration in seconds")
    priority: Priority = Field(default=Priority.MEDIUM, description="Watch priority")
    reason: str = Field(
...
class WatchList(BaseModel):
    ...
    items: list[WatchListItem] = Field(
        default_factory=list, description="Prioritized list of videos"
    )
    criteria: str = Field(description="Criteria used to select these videos")
    gaps: list[str] = Field(
        default_factory=list, description="Topics not covered - 'what's missing' messaging"
    )
...
class SynthesizeRequest(BaseModel):
    ...
    synthesis_type: SynthesisType = Field(
        alias="synthesisType", description="Type of synthesis to perform"
    )
    query: str = Field(
    ...
    max_items: int = Field(
        default=10, ge=1, le=50, alias="maxItems", description="Maximum number of items to include"
    )
...
class SynthesizeResponse(BaseResponse):
    ...
    watch_list: WatchList | None = Field(
        default=None, alias="watchList", description="Watch list if synthesisType is 'watch_list'"
    )
