...
class SearchService:
    ...
    def _build_scope_filter(self, scope: QueryScope | None) -> list[Any]:
        ...
        if scope.date_range:
            if scope.date_range.from_date:
                conditions.append(
                    Video.publish_date
                    >= datetime.combine(scope.date_range.from_date, datetime.min.time())
                )
            if scope.date_range.to_date:
                conditions.append(
                    Video.publish_date
                    <= datetime.combine(scope.date_range.to_date, datetime.max.time())
                )
    ...
    async def _get_video_ids_for_scope(self, scope: QueryScope | None) -> list[UUID] | None:
        ...
        if scope and scope.facets:
            ...
            facet_query = (
                select(VideoFacet.video_id).where(VideoFacet.facet_id.in_(scope.facets)).distinct()
            )
        ...
        with _tracer.start_as_current_span(
            ...
            try:
                ...
                if rows:
                    span.set_attribute(
                        "search.top_score",
                        float(rows[0].distance) if hasattr(rows[0], "distance") else 0,
                    )
        ...
        if date_row:
            if date_row.earliest and date_row.latest:
                date_range = CoverageDateRange(
                    earliest=date_row.earliest.date()
                    if hasattr(date_row.earliest, "date")
                    else date_row.earliest,
                    latest=date_row.latest.date()
                    if hasattr(date_row.latest, "date")
                    else date_row.latest,
                )
