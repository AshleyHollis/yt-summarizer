...
class CopilotService:
    ...
    async def query(self, request: CopilotQueryRequest) -> CopilotQueryResponse:
        ...
        with _tracer.start_as_current_span(
                ...
                if not ai_settings.use_video_context:
                    ...
                    if not ai_settings.use_llm_knowledge:
                        ...
                        return CopilotQueryResponse(
                            ...
                            followups=[
                                "Enable 'Your Videos' to search your library",
                                "Enable 'AI Knowledge' to use general knowledge",
                            ],
                    ...
                    return await self._generate_llm_only_response(
                        request, ai_settings, correlation_id
                    )
                else:
            ...
            try:
                ...
                span.set_attribute(
                    "copilot.llm.confidence", llm_result.get("confidence", "unknown")
                )
    ...
    async def _generate_llm_only_response(
        ...
        if not followups:
            try:
                ...
                logger.warning(f"Failed to generate follow-ups: {e}")
                followups = [
                    "Would you like to search your video library for more specific information?"
                ]
    ...
    async def _retrieve_legacy(
        ...
        relevant_segments = [seg for seg in segments if seg.score <= MAX_DISTANCE_FOR_RELEVANCE]
