...
def sample_user_message():
    ...
    return {"id": str(uuid4()), "role": "user", "content": "What videos do I have about Python?"}
        ...
        "toolCalls": [
            {
                "id": tool_call_id,
                "type": "function",
                "function": {"name": "queryLibrary", "arguments": '{"query":"Python videos"}'},
            }
        ],
    }
...
@pytest.fixture
def complete_thread_messages(
    sample_user_message, sample_assistant_with_tool_calls, sample_tool_result
):
    """Create a complete thread with user -> assistant(toolCalls) -> tool(result)."""
    return [sample_user_message, sample_assistant_with_tool_calls, sample_tool_result]
...
class TestThreadMessageStructure:
    ...
    def test_assistant_message_has_required_toolcalls_fields(
        self, sample_assistant_with_tool_calls
    ):
    ...
    def test_tool_result_has_matching_toolcallid(
        self, sample_assistant_with_tool_calls, sample_tool_result
    ):
...
class TestThreadsAPI:
    ...
    @pytest.mark.skip(
        reason="Requires database persistence - mock doesn't retain data between requests"
    )
    @pytest.mark.asyncio
    async def test_create_thread_with_tool_calls(self, client, complete_thread_messages):
        """POST /api/v1/threads/messages should create thread with toolCalls preserved."""
        response = client.post(
            "/api/v1/threads/messages",
            json={"messages": complete_thread_messages, "title": "Test Thread with Tool Calls"},
        )
    ...
    @pytest.mark.asyncio
    async def test_get_thread_preserves_tool_call_structure(self, client, complete_thread_messages):
        """GET /api/v1/threads/{id} should return messages with toolCalls intact."""
        # First create a thread
        create_response = client.post(
            "/api/v1/threads/messages",
            json={"messages": complete_thread_messages, "title": "Test Thread for Retrieval"},
        )
    ...
    @pytest.mark.asyncio
    async def test_update_thread_with_additional_tool_calls(self, client, complete_thread_messages):
        """PUT /api/v1/threads/{id}/messages should preserve new tool calls."""
        # Create initial thread
        create_response = client.post(
            "/api/v1/threads/messages",
            json={"messages": complete_thread_messages, "title": "Thread to Update"},
        )
        thread_id = create_response.json()["thread_id"]
        ...
        updated_messages = complete_thread_messages + [
            ...
            },
            {
                "id": str(uuid4()),
                "role": "tool",
                "content": '{"answer":"follow up result"}',
        ...
        update_response = client.put(
            f"/api/v1/threads/{thread_id}/messages", json={"messages": updated_messages}
        )
    ...
    async def test_thread_with_multiple_simultaneous_tool_calls(self, client):
        ...
        create_response = client.post(
            "/api/v1/threads/messages", json={"messages": messages, "title": "Multi-Tool-Call Test"}
        )
...
class TestThreadDisplayFormatting:
    ...
    def test_messages_with_tool_calls_can_be_rendered(self, complete_thread_messages):
        ...
        has_tool_calls = (
            "toolCalls" in assistant_msg and len(assistant_msg.get("toolCalls", [])) > 0
        )
