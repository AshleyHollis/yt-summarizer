...
class TestLLMServiceErrors:
    ...
    async def test_embedding_api_connection_error(self, mock_db_session, mock_llm_service):
        ...
        assert (
            "try again" in response.answer.lower()
            or "unable" in response.answer.lower()
    ...
    async def test_llm_generation_failure_after_successful_search(
        ...
        mock_llm_service.generate_answer = AsyncMock(side_effect=Exception("LLM generation failed"))
...
class TestSearchServiceErrors:
    ...
    async def test_database_connection_failure(self, mock_db_session, mock_llm_service):
        ...
        assert (
            "search failed" in response.answer.lower()
            or "try again" in response.answer.lower()
            or "don't have" in response.answer.lower()
        )
...
class TestNeighborsEndpointErrors:
    ...
    async def test_neighbors_with_malformed_uuid(self, async_client):
        ...
        response = await async_client.get("/api/v1/copilot/neighbors/not-a-valid-uuid")
    ...
    async def test_neighbors_with_nonexistent_video(self, async_client):
        ...
        response = await async_client.get(f"/api/v1/copilot/neighbors/{uuid4()}")
...
class TestConcurrency:
    ...
    async def test_concurrent_queries(self, async_client):
        ...
        tasks = [make_query(f"Concurrent query {i}") for i in range(5)]
    ...
    async def test_concurrent_search_requests(self, async_client):
        ...
        tasks = [make_search(f"Search query {i}") for i in range(5)]
...
class TestUncertaintyDetection:
    ...
    @pytest.mark.asyncio
    async def test_uncertainty_with_low_confidence_results(self, mock_db_session, mock_llm_service):
...
class TestLLMNullStringHandling:
    ...
    async def test_valid_uncertainty_preserved(self):
        ...
        assert (
            uncertainty_value
            == "The provided evidence contains no information about cooking pasta."
        )
...
class TestLLMServiceUncertaintyIntegration:
    ...
    async def test_generate_answer_handles_null_string(self):
        ...
        with patch.object(LLMService, "client", new_callable=lambda: MagicMock()) as mock_client:
            ...
            result = await llm_service.generate_answer(
                ...
                evidence=[
                    {"text": "Heavy clubs training info", "video_title": "Heavy Clubs Video"}
                ],
            )
    ...
    async def test_generate_answer_preserves_valid_uncertainty(self):
        ...
        with patch.object(LLMService, "client", new_callable=lambda: MagicMock()) as mock_client:
            ...
            result = await llm_service.generate_answer(
                query="Tell me about cooking pasta", evidence=[]
            )
...
class TestRelevanceFiltering:
    ...
    async def test_all_irrelevant_segments_returns_no_information(
        ...
        assert (
            "don't have" in response.answer.lower() or "no information" in response.answer.lower()
        )
        assert response.video_cards == []
        assert response.evidence == []
...
class TestModerateRelevanceThreshold:
    ...
    @pytest.mark.asyncio
    async def test_truly_irrelevant_content_still_filtered(self, mock_db_session, mock_llm_service):
...
class TestQueryExpansion:
    ...
    @pytest.mark.asyncio
    async def test_query_expansion_generates_related_terms(self, mock_db_session, mock_llm_service):
    ...
    @pytest.mark.asyncio
    async def test_query_expansion_deduplicates_segments(self, mock_db_session, mock_llm_service):
    ...
    @pytest.mark.asyncio
    async def test_query_expansion_fallback_on_failure(self, mock_db_session, mock_llm_service):
        ...
        mock_llm_service.expand_query = AsyncMock(side_effect=Exception("LLM expansion failed"))
        mock_llm_service.get_embedding = AsyncMock(return_value=[0.1] * 1536)
        mock_llm_service.generate_answer = AsyncMock(
            return_value={
