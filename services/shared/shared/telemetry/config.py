...
def configure_telemetry(
    ...
    otlp_endpoint = os.environ.get(
        "OTEL_EXPORTER_OTLP_ENDPOINT", os.environ.get("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT")
    )
    ...
    print(
        f"[TELEMETRY] OTLP endpoint: {otlp_endpoint}, protocol: {otlp_protocol}",
        file=sys.stderr,
        flush=True,
    )
    if otlp_headers:
        print(
            f"[TELEMETRY] OTLP headers configured: {list(otlp_headers.keys())}",
            file=sys.stderr,
            flush=True,
        )
    ...
    if not otlp_endpoint:
        print(
            "[TELEMETRY] No OTLP endpoint configured, telemetry disabled",
            file=sys.stderr,
            flush=True,
        )
        return False
    ...
    try:
        ...
        if otlp_protocol == "http/protobuf":
            ...
            if traces_endpoint.startswith("https://"):
                ...
                if ssl_cert:
                    ...
                    print(
                        f"[TELEMETRY] Using SSL cert for session: {ssl_cert}",
                        file=sys.stderr,
                        flush=True,
                    )
                else:
                    ...
                    print(
                        "[TELEMETRY] WARNING: Disabling SSL verification (no cert found)",
                        file=sys.stderr,
                        flush=True,
                    )
        ...
        try:
            ...
            print(
                "[TELEMETRY] OpenAI instrumentation enabled (Gen AI traces)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
        ...
        try:
            ...
            print(
                "[TELEMETRY] SQLAlchemy instrumentation enabled (DB traces)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
            print(
                "[TELEMETRY] SQLAlchemy instrumentation not available", file=sys.stderr, flush=True
            )
        except Exception as e:
            print(
                f"[TELEMETRY] SQLAlchemy instrumentation failed: {e}", file=sys.stderr, flush=True
            )
        ...
        try:
            ...
            print(
                "[TELEMETRY] HTTPX instrumentation enabled (HTTP client traces)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
        ...
        try:
            ...
            print(
                "[TELEMETRY] Logging instrumentation enabled (log-trace correlation)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
        ...
        try:
            ...
            print(
                "[TELEMETRY] Azure SDK tracing enabled (Storage Queue/Blob traces)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
            print(
                "[TELEMETRY] Azure SDK tracing not available (install azure-core-tracing-opentelemetry)",
                file=sys.stderr,
                flush=True,
            )
        except Exception as e:
        ...
        try:
            ...
            print(
                "[TELEMETRY] System metrics instrumentation enabled (CPU, memory, runtime)",
                file=sys.stderr,
                flush=True,
            )
        except ImportError:
            print(
                "[TELEMETRY] System metrics not available (install opentelemetry-instrumentation-system-metrics)",
                file=sys.stderr,
                flush=True,
            )
        except Exception as e:
            print(
                f"[TELEMETRY] System metrics instrumentation failed: {e}",
                file=sys.stderr,
                flush=True,
            )
        ...
        print(
            f"[TELEMETRY] OpenTelemetry configured successfully for {service_name}",
            file=sys.stderr,
            flush=True,
        )
...
def _configure_metrics_exporter(
    ...
    try:
        ...
        print(
            f"[TELEMETRY] Metrics exporter configured (interval: {export_interval_ms}ms)",
            file=sys.stderr,
            flush=True,
        )
        return True
