...
run_deploy_with_timeout() {
  ...
  echo "::group::Deployment attempt $attempt of $MAX_ATTEMPTS (${timeout}s timeout)"
  ...
  if timeout "${timeout}s" npx --yes @azure/static-web-apps-cli deploy \
    ...
    echo "::notice::Deployment succeeded on attempt $attempt (${elapsed}s elapsed)"
    echo "::endgroup::"
    return 0
  else
    local exit_code=$?
