...
# Check if file exists (may not exist yet for new preview overlays)
if [ ! -f "$FILE_TO_COMMIT" ] && [ "$OPERATION" != "update-preview" ]; then
  echo "::error::File not found: $FILE_TO_COMMIT"
  exit 1
fi

# For preview, the file is generated by update-preview.sh, so check if it exists now
if [ "$OPERATION" == "update-preview" ] && [ ! -f "$FILE_TO_COMMIT" ]; then
