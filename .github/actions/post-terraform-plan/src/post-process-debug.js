...
function applyIndentation(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (resourceMatch) {
      ...
      console.log(`DEBUG: Resource header detected, resourceMarker="${resourceMarker}", trimmed="${trimmed}"`);
    ...
    if (bareContent === '}' && inResourceBlock && depth === 1) {
      ...
      depth = 0;
      console.log("DEBUG closing brace: resourceMarker =", resourceMarker); result.push(`${resourceMarker} }`);
      continue;
    }
